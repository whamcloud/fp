...
export const over = curry3(
  (lens, fn, xs) => lens(ys => getIdentity(fn(ys)))(xs).value
);
...
export const mapped = curry2((fn, x) =>
  getIdentity(map(flow(fn, getValue), x)));
...
export const eq = curry2(
  (a, b) => a && typeof a.equals === 'function' ? a.equals(b) : a === b
);
